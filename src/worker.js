...
const landingHtml = `<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>論文指導工作台</title><style>
:root{--ink:#192636;--muted:#657486;--line:#e3e9ee;--bg:#f5f7f8;--blue:#176b87}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font:16px/1.6 system-ui,-apple-system,"Noto Sans TC",sans-serif;display:grid;place-items:center;min-height:100vh;padding:24px}.card{background:#fff;border:1px solid var(--line);border-radius:14px;padding:24px;max-width:560px;width:100%}h1{margin:0 0 8px;font-size:26px}.muted{color:var(--muted);margin:0 0 16px}.links{display:grid;grid-template-columns:1fr 1fr;gap:12px}a{display:block;text-align:center;text-decoration:none;background:var(--blue);color:#fff;border-radius:10px;padding:12px 10px;font-weight:700}@media(max-width:560px){.links{grid-template-columns:1fr}}</style></head><body><section class="card"><h1>論文指導工作台</h1><p class="muted">請選擇進入角色</p><div class="links"><a href="/student">學生工作台</a><a href="/supervisor">教授收件匣</a></div></section></body></html>`;
...
function student() {
  ...
  var resolved = state.decisions.filter(function(x) { return x.status !== 'pending'; });
  ...
  var decHtml = resolved.map(function(d) {
  ...
  if (resolved.length === 0) decHtml = '<div class="empty">尚無已完成決策</div>';
  ...
  app.innerHTML = '<section class="hero"><div><h2>你好，' + esc(state.profile.studentName) + '</h2><p>' + esc(state.profile.title) + ' · 目標口試 ' + date(state.profile.targetDefense) + '</p></div><button onclick="decisionForm()">＋ 提出決策請求</button></section><section class="grid"><div class="card"><div class="stat">' + pending.length + '</div><div class="muted">等待教授決策</div></div><div class="card"><div class="stat">' + open.length + '</div><div class="muted">未完成待辦</div></div><div class="card"><div class="stat">' + riskMs.length + '</div><div class="muted">黃／紅燈里程碑</div></div></section><section class="grid two" style="margin-top:16px"><div class="card"><h3>Thesis Roadmap</h3><div class="list">' + msHtml + '</div></div><div class="card"><h3>本週 Brief</h3><p>' + esc(state.weeklyBrief.summary) + '</p><div class="muted">待教授決策：' + pending.length + ' 項</div><div class="muted">本週完成</div><ul>' + winsHtml + '</ul><div class="muted">下一步</div><ul>' + nextStepsHtml + '</ul></div></section><section class="grid two" style="margin-top:16px"><div class="card"><h3>Decision Log</h3><div class="list">' + decHtml + '</div></div><div class="card"><h3>我的 Action Items</h3><div class="list">' + actHtml + '</div></div></section>';
}
